  ...
  return (
    <div className="space-y-5 px-4 pb-28 pt-5">
      ...
      <div className="flex items-center gap-3">
        <div className="grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br from-cyan-400 to-indigo-500 text-lg font-extrabold text-white">
          {lvl.level}
        </div>
        ...
        <div className="flex flex-col items-center rounded-2xl bg-amber-400/15 px-3 py-1.5">
          ...
          <span className="text-sm font-extrabold text-amber-600">{s.streak}</span>
        </div>
      </div>
      ...
      <Card className="flex items-center gap-5 p-5">
        <Ring pct={(day / s.planDays) * 100} color="#22d3ee">
          <div className="text-center">
            ...
            <div className="text-xs text-slate-500">/ {s.planDays}</div>
          </div>
        </Ring>
        <div className="flex-1">
          <div className="text-sm text-slate-600">一個月衝刺計畫</div>
          <div className="mt-1 text-2xl font-extrabold">
            {s.introduced.toLocaleString()}
            <span className="text-base font-medium text-slate-500"> / {TOTAL.toLocaleString()} 字</span>
          </div>
          <div className="mt-2">
            <Bar pct={planPct} color="from-emerald-400 to-cyan-500" />
          </div>
          <div className="mt-1 text-xs text-slate-500">已解鎖 {planPct}% · 精熟 {mastery}%</div>
        </div>
      </Card>
      ...
      <Card className="overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-3 px-5">
          <MiniStat emoji="✨" n={newToday} label="新單字" tint="text-cyan-600" />
          <MiniStat emoji="🔁" n={due} label="待複習" tint="text-amber-600" />
        </div>
        <div className="p-5 pt-4">
          <button
            onClick={go(() => buildDaily(), "今日任務")}
            disabled={newToday === 0 && due === 0}
            className="w-full rounded-2xl bg-gradient-to-r from-cyan-400 to-indigo-500 py-4 text-base font-extrabold text-white disabled:opacity-40"
          >
          ...
          {due > 0 && (
            <button
              ...
              className="mt-2.5 w-full rounded-2xl bg-slate-100 py-3 text-sm font-bold text-amber-600 ring-1 ring-slate-200"
            >
              只複習到期單字（{due}）
            </button>
          )}
      ...
      {/* practice modes */}
      <div>
        <div className="mb-2 px-1 text-sm font-bold text-slate-600">自由練習</div>
        <div className="grid grid-cols-2 gap-3">
          {practice.map((p) => (
            <button
              ...
              className="flex items-center gap-3 rounded-2xl bg-white p-4 text-left ring-1 ring-slate-200 disabled:opacity-40"
            >
              <span className="text-2xl">{p.emoji}</span>
              <div>
                <div className="font-bold">{p.label}</div>
                <div className="text-xs text-slate-500">隨機 15 題</div>
              </div>
            </button>
          ))}
        </div>
        {s.introduced < 4 && (
          <p className="mt-2 px-1 text-xs text-slate-400">先學幾個新單字就能解鎖自由練習。</p>
        )}
      </div>
